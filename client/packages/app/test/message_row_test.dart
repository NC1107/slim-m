...
void main() {
  ...
  group('reactions', () {
    testWidgets('a chip shows the real count and reflects whether you reacted',
      ...
      await tester.pumpWidget(_harness(MessageRow(
        ...
        onCancelEdit: () {},
        reactions: const [
          // Escaped, not literal: the hygiene gate forbids emoji codepoints in
          // client source, and these are user content (a reaction), not chrome.
  ...
  testWidgets('the emoji panel survives the pointer leaving the message row',
    ...
    // The failure this exists for: the panel occludes the row, the pointer
    // leaves the MouseRegion, and the button used to unmount with the panel.
