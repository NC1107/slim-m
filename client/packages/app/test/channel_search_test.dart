...
void main() {
  testWidgets('a search in flight shows a spinner, not "no matches"',
      (tester) async {
    await tester.pumpWidget(_harness(const ChannelSearchResults(
      results: null,
      knownUsernames: {},
      loading: true,
      failed: false,
  ...
  testWidgets('a failed search says so and offers a working retry',
      (tester) async {
    var retried = false;
    await tester.pumpWidget(_harness(ChannelSearchResults(
      results: null,
  ...
  testWidgets('a 403 explains the denial and offers no retry', (tester) async {
    ...
    expect(find.text('You do not have permission to search this channel.'),
        findsOneWidget);
