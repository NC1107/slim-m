...
Widget _harness({
  ...
  return ProviderScope(
    ...
    child: MaterialApp(
      theme: buildTheme(Brightness.light, AppTokens.light).copyWith(
        platform: platform,
      ),
      home: Scaffold(
        body: Column(
...
void main() {
  ...
  testWidgets('a hardware return on a touch platform sends exactly once',
    ...
    expect(sends.count, 1,
        reason: 'the shortcut and the send action must not both be live on '
            'one platform');
  });

  testWidgets('on desktop the field keeps the newline action and its shortcut',
      (tester) async {
    await tester.pumpWidget(_harness(
      controller: controller,
      sends: sends,
  ...
  testWidgets('the shift + enter hint is hidden on touch and shown on desktop',
      (tester) async {
    await tester.pumpWidget(_harness(
      controller: controller,
      sends: sends,
  ...
  testWidgets('a picked emoji lands at the caret, not at the end',
    ...
    await tester.tap(find.descendant(
        of: find.byType(EmojiGrid), matching: find.text(glyph)));
    await tester.pumpAndSettle();

    expect(controller.text, 'hi$glyph there',
        reason: 'nothing asserted the picked emoji reached the field at all');
    expect(controller.selection.baseOffset, 2 + glyph.length,
        reason: 'the caret must follow what was inserted, or the next '
  ...
  group('touch density', () {
    testWidgets('a phone gets 44pt controls and keeps a usable field',
      ...
      for (final element in find.byType(AppIconButton).evaluate()) {
        expect(tester.getSize(find.byWidget(element.widget)).shortestSide,
      ...
      expect(
          tester.getSize(find.byType(ComposerField)).width, greaterThan(160.0));
    });
