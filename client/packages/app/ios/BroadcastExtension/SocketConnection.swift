...
// it in turn from Jitsi Meet. Kept close to the original on purpose: the app
// side of this socket is flutter_webrtc's `FlutterSocketConnection`, and the
// framing either matches it exactly or produces no video at all.
import Foundation
import OSLog

/// The extension's end of the unix domain socket in the shared App Group
/// container. The app listens; this connects.
class SocketConnection: NSObject {
  ...
  /// Returns false rather than throwing when the app has not created the
  /// socket yet, which is the normal case for the first few polls.
  func open() -> Bool {
...
extension SocketConnection: StreamDelegate {
  func stream(_ aStream: Stream, handle eventCode: Stream.Event) {
    ...
    case .errorOccurred:
      os_log(.debug, log: broadcastLogger, "client stream error occurred")
      close()
...
private extension SocketConnection {
  ...
  func connectSocket() -> Bool {
    ...
    guard status == noErr else {
      os_log(.debug, log: broadcastLogger, "failure: connect returned %d", status)
      return false
    }
    return true
  }
