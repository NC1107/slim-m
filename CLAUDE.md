...
- `actionlint` (pinned by image digest) over every workflow, which also shellchecks the inline `run:` blocks, and `shellcheck` over every tracked `*.sh`. Both were green when they landed, so either failing means the change under review introduced it.
