...
**The identical gap existed in the rpm for the identical reason**, fixed in the same change: `libtray_manager_plugin.so` sits inside the spec's own `__requires_exclude` pattern (`lib.*_plugin\.so`), the same exclusion that keeps `libwebrtc.so`'s internals out of rpm's dependency namespace, so rpm's ELF scan never saw this one either.
One `Requires: libayatana-appindicator3.so.1()(64bit)` line closes it; `packaging/rpm/README.md`'s dependency table has the full reasoning, including the Ubuntu-versus-Fedora soname difference that made this easy to get backwards (see below).
