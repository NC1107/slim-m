...
This closes the first and third; Android's Notification Channels are still open and untouched here.
Read this before touching `sounds.py`'s `CALLKIT_RINGTONE`, `NotificationSound.swift`, or either `CXProviderConfiguration` in the Runner target.

**The ringtone is generated by the same pipeline as the seven, and deliberately is not one of them.**
`sounds.py`'s `CALLKIT_RINGTONE` reuses `call_ring`'s own two-note figure, repeated three times with a real pause between each repeat rather than the seven's single one-shot cycle, because `CXProviderConfiguration.ringtoneSound` is a standalone file CallKit itself loops for the life of the incoming-call screen, not an in-app chime `audioplayers` fires once.
It is rendered by the same `synth.render`/`normalise`/`write_wav` and diffed by the same `audio-ci` `git diff -- assets/audio`, but it is not added to `SOUNDS`: `test_every_sound_exists` asserts the family is exactly seven, and `assets/audio/generate.py`'s `main()` renders it as a separate call after the family loop instead.
`CallKitRingtone` in `test_sounds.py` covers what SoundFamily's own tests do not reach for it: Apple's 30-second ringtone ceiling, that it is not accidentally in `SOUNDS`, and that its committed bytes match `CALLKIT_RINGTONE`'s own notes.

**Two `CXProviderConfiguration`s exist, and only one of them can ever actually ring.**
`VoipCallHandler.swift`'s `VoipPushRegistrar` is the one that calls `reportNewIncomingCall`, so its `ringtoneSound` is the setting that matters - and per that file's own doc comment, nothing in this app constructs a `VoipPushRegistrar` today, so the ringtone has no live caller yet either.
`VoiceCallReporter.swift`'s `VoiceCallChannel` only ever reports an *outgoing* call via `CXStartCallAction`, which CallKit does not ring for, so its own `ringtoneSound` is set only for parity in case the two providers are ever consolidated, per that file's own existing doc comment about that being a decision for whoever wires the inbound path up.
Both share one `callKitRingtoneFileName` constant so they cannot drift.

**The ringtone WAV is bundled by a native pbxproj resource, never through the Flutter asset pipeline.**
`UNNotificationSound(named:)` and `CXProviderConfiguration.ringtoneSound` both resolve a bare filename against the app's own main bundle, not against `Runner.app/Frameworks/App.framework/flutter_assets/...` where `pubspec.yaml`'s asset entries land - so `direct_message.wav`, `group_message.wav` and `callkit_ringtone.wav` are referenced from `project.pbxproj`'s new `Audio` group by relative path straight back to `assets/audio/notifications/`, the same single source of truth, and copied into Runner's own `Resources` build phase.
The relative path (`../../../../assets/audio/notifications/...`) mirrors the existing `Flutter` group's own file references, which already resolve outside the project directory the identical way; `lastKnownFileType = audio.wav` matches the `audio.aiff` Xcode itself writes for a sibling case in the vendored `flutter_local_notifications` example project, checked directly rather than guessed.

**The wire cannot tell a mention apart from a plain channel message, and this does not pretend otherwise.**
`crates/slimm-server/src/push/envelope.rs`'s `PushKind` has exactly one variant, `Message`, for every message push - DM, mention, or ordinary post alike - and the sealed plaintext carries no mention flag either.
`NotificationSound.decide` therefore only distinguishes what the decrypted preview actually carries: `channel` present (a named channel, so `group_message.wav`) or absent (a DM or a thread reply, both of which carry no channel name by design per `PushEnvelope.applied(to:)`'s own doc comment, so `direct_message.wav`).
A mention plays the group sound, not a dedicated one - closing that needs a wire change, which is out of this change's scope (the envelope format belongs to a sibling task), and is named here rather than silently approximated.
A content-free envelope, or one that failed to decode, leaves `content.sound` untouched instead of guessing.

**A live call silences every push, including ones this extension could not even read.**
`NotificationSound.decide`'s call-activity check runs before anything else, and `NotificationService.swift`'s `withSound` is applied to both `didReceive`'s ordinary path and `serviceExtensionTimeWillExpire`'s timeout fallback, so a call in progress silences a push whether or not the sealed envelope ever opened.
The mechanism is `CXCallObserver().calls`, read directly rather than through a shared App Group: that property reflects CallKit's already-known state the instant the observer is constructed, it does not wait for a delegate callback, so the read stays synchronous like everything else this extension does, and it needs no App Group and no portal capability - a real constraint, since `NotificationService.entitlements`' own comment already says this process deliberately holds no App Group, and adding one would mean regenerating a provisioning profile through the Apple Developer portal, which nothing here can do.
It reports a call CallKit is tracking for any app, including the system Phone app, since the question is whether a chime would step on audio already playing, not only on a call this app itself joined.

**Unverified on a real device, the same evidentiary bar this client's other untested-on-device iOS surfaces already carry.**
Nothing here can confirm CallKit actually rings with the bundled file, that a chime actually stays silent during a live call, or that `CXCallObserver` behaves as documented inside a Notification Service Extension specifically rather than only in a main app - all three are reasoned from Apple's documented API semantics and covered by `NotificationSoundTests.swift`'s unit tests against a fake `CallActivityChecking`, not by a device run.
`client-ios-ci`'s `pbxproj-registration` job and `ios unit tests (callkit invariant)` job compiling and linking both new files, and `hygiene`'s `ios notification service extension is wired up` check passing against the edited `project.pbxproj`, are the whole of the evidence.
...
Flutter 3.47.0 stable (Dart 3.13.0) is installed at `~/development/flutter`, on PATH via `.zshrc` and `.bashrc`. `flutter doctor` reports no issues.
Every workflow pins that same version, and `client/pubspec.lock` is resolved under it: the three move together or CI and a local run disagree about what they are testing.
