...
- **A DM's first message never reached the recipient live.** `MessageStore` has no channel foreign key, so a `MessageCreated` frame for a channel the client had never fetched landed silently and `_advanceCursor` no-opped. `SyncController._applyServerEvent` now checks `store.hasChannel` first and materialises the channel (`_refreshChannelsOnce`, debounced against a burst) before applying the message. The same gap silently hid any newly created channel until reconnect; ~~there is still no `channel.created` event anywhere in the wire protocol, so a next contributor adding one should also delete this workaround.~~
  ...
  Settled 2026-08-11, later the same day: it cannot go, and the permission gate was never the blocker.
  `open` (`http/dms.rs`) calls `Store::open_dm` and returns straight away - it never touches `state.hub` at all, unlike `POST /channels`' `create` (`http/channels.rs`), which publishes `Event::ChannelCreated` in the same handler that creates the row.
  So a DM's first-contact recipient gets no `channel.created` frame to race against a `MessageCreated` one; there is nothing published for the workaround's `_channelRefresher.refreshOnce` fallback to have become redundant with.
  Checked anyway, since it would have mattered had the first check gone the other way: had `open` published `ChannelCreated`, `http::ws::authorization::authorize` would have let it through fine - the gate is the ordinary `permissions_in_channel` check on the channel id the event carries, and a DM's evaluator (`store::dms::dm_permissions`) grants `VIEW_CHANNEL` to both participants unconditionally, so the recipient would see it.
  And the client already has a case for it (`sync_controller.dart`'s `ChannelCreated(:final channel): store.upsertChannels([channel])`), so the wiring on the receiving end is not the gap either.
  The gap is only ever the one line: nothing calls `hub.publish` for a DM open.
  `crates/slimm-server/tests/dm_open_publishes_no_event.rs` is the standing proof, subscribing to the hub directly around a real `POST /dms/{id}` and asserting nothing arrives; mutation-tested by adding the missing publish call by hand, which fails exactly that test and nothing else in `dms.rs`, then reverted.
  The workaround stays, and the note above should be read as: the wire event existing was never sufficient on its own, since a `ChannelCreated` genuinely reaching every recipient needs it published from every place a channel-shaped thing can come into being a caller was not already watching, and `open_dm` is one such place that still publishes nothing.
