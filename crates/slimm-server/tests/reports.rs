...
/// A report about a user carries no author, because there is no message. The
/// same null the client renders as "no longer on this Space" rather than as a
/// lookup still in flight.
#[tokio::test]
async fn a_user_report_carries_no_subject_author() {
    let (store, _guard) = new_store().await;
    let app = app(store.clone());
    let (admin_token, _admin_id) = register(&store, "alice").await;
    ...
    .await;
    assert!(listed.as_array().unwrap()[0]["subject_author_id"].is_null());
}
