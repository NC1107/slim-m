...
use crate::fixtures::{
    app, general, id, member, move_op, new_store, new_store_and_pool, post_object, register,
    stroke, submit_op,
};

async fn bounds(pool: &sqlx::SqlitePool, object_id: &str) -> (f64, f64, f64, f64, i64) {
...
#[tokio::test]
async fn a_member_may_move_their_own_object_with_no_special_bit() {
    ...
    let (status, body) = submit_op(
        ...
        channel,
        &bob_token,
        move_op(&id(), &object_id, (500.0, 600.0, 30.0, 40.0)),
    )
    .await;
    assert_eq!(status, StatusCode::CREATED, "{body}");
    assert_eq!(body["op"]["affected"], 1);
    assert_eq!(body["op"]["kind"], "move");

    let (x, y, w, h, _) = bounds(&pool, &object_id).await;
    assert_eq!((x, y, w, h), (500.0, 600.0, 30.0, 40.0));
}

/// The shape `a_member_cannot_remove_anothers_object_but_a_moderator_can`
/// already proves for `remove`, extended to `move`.
#[tokio::test]
async fn a_member_cannot_move_anothers_object_but_a_moderator_can() {
    ...
    assert_eq!(
        ...
        "an ordinary member cannot move someone else's object: {body}"
    );
...
#[tokio::test]
async fn a_timed_out_member_cannot_move_their_own_object() {
    let (store, _guard) = new_store().await;
    ...
    let (status, body) = submit_op(
        ...
        channel,
        &bob_token,
        move_op(&id(), &object_id, (9.0, 9.0, 9.0, 9.0)),
    )
    .await;
    assert_eq!(
        status,
        StatusCode::FORBIDDEN,
        "a timeout must freeze move the same way it freezes placing: {body}"
    );
}

/// Racing an erase: an honest retry deserves a truthful answer, not an
/// error, the same reasoning `PlaceError::Removed` already documents.
#[tokio::test]
async fn moving_an_already_removed_object_answers_affected_zero() {
    let (store, _guard) = new_store().await;
    ...
    let (status, body) = submit_op(
        ...
        channel,
        &token,
        move_op(&id(), &object_id, (1.0, 1.0, 1.0, 1.0)),
    )
    .await;
    assert_eq!(status, StatusCode::CREATED, "{body}");
...
#[tokio::test]
async fn bounds_outside_the_world_or_over_the_extent_ceiling_are_refused() {
    let (store, _guard) = new_store().await;
    let (token, _) = register(&store, "root").await;
    let channel = general(&store).await;
    let app = app(store.clone());
    ...
    let (status, body) = submit_op(
        ...
        channel,
        &token,
        move_op(&id(), &object_id, (0.0, 0.0, 9_000.0, 9_000.0)),
    )
    .await;
    assert_eq!(status, StatusCode::BAD_REQUEST, "{body}");
}

/// A move must not disturb `z_index` (paint order) or `props` (a stroke's
/// own points, or an image's attachment reference) - only the bounding box.
#[tokio::test]
async fn moving_does_not_touch_z_index_or_props() {
    let (store, pool, _guard) = new_store_and_pool().await;
    let (token, _) = register(&store, "root").await;
    let channel = general(&store).await;
    let app = app(store.clone());
    ...
    let (_, placed) = post_object(&app, channel, &token, stroke(&id())).await;
    let object_id = placed["id"].as_str().unwrap().to_owned();
    let (_, _, _, _, before_z) = bounds(&pool, &object_id).await;

    submit_op(
        &app,
        channel,
        &token,
        move_op(&id(), &object_id, (111.0, 222.0, 5.0, 6.0)),
    )
    .await;

    let (x, y, w, h, after_z) = bounds(&pool, &object_id).await;
    assert_eq!((x, y, w, h), (111.0, 222.0, 5.0, 6.0));
    assert_eq!(before_z, after_z, "z_index must survive a move untouched");
...
#[tokio::test]
async fn a_replayed_move_returns_the_stored_op_and_writes_nothing_new() {
    let (store, _guard) = new_store().await;
    let (token, _) = register(&store, "root").await;
    let channel = general(&store).await;
    let app = app(store.clone());
    ...
    let (first_status, first_body) = submit_op(
        &app,
        channel,
        &token,
        move_op(&op_id, &object_id, (10.0, 10.0, 10.0, 10.0)),
    )
    .await;
    ...
    let (second_status, second_body) = submit_op(
        ...
        channel,
        &token,
        // Different bounds than the first call: a replay must ignore them.
        move_op(&op_id, &object_id, (99.0, 99.0, 99.0, 99.0)),
    )
    .await;
...
#[tokio::test]
async fn account_deletion_nulls_the_actor_id_of_a_move_op() {
    let (store, _guard) = new_store().await;
    ...
    assert_eq!(page.ops.len(), 1, "the move op itself survives");
    assert_eq!(
