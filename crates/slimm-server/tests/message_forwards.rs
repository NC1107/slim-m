...
async fn new_store() -> (Store, support::TestDbGuard) {
    let (path, guard) = support::TestDbGuard::new("slimm-message-forwards");
    let config = Config {
        port: 0,
        database_path: path,
        hash_concurrency: 2,
...
async fn register(store: &Store, username: &str, display: &str) -> String {
    let account = store
...
async fn open_deployment(store: &Store) -> (slimm_server::ids::ChannelId, String) {
    store
        .create_role(
            "everyone",
...
async fn post(app: &Router, channel: &str, token: &str, body: Value) -> axum::response::Response {
    app.clone()
        .oneshot(request(
            "POST",
...
fn forward_body(content: &str, origin: &Value) -> Value {
    json!({
