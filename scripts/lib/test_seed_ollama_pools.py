...
"""Unit coverage for the pooled, ollama-generated `Corpus`.
...
and answered with an empty or partial `Corpus` rather than raised. The
shared wire plumbing (`_request`, `_reachable`, the on-disk cache) lives in
and is tested by `test_seed_ollama.py`; this file covers the corpus-shaped
...
class CorpusTest(unittest.TestCase):
    def test_a_fresh_corpus_with_nothing_in_it_is_empty(self):
        self.assertTrue(seed_ollama_pools.Corpus().is_empty())

    def test_one_populated_pool_is_enough_to_not_be_empty(self):
        self.assertFalse(seed_ollama_pools.Corpus(short=["hi"]).is_empty())

    def test_round_trips_through_as_dict_and_back(self):
        original = seed_ollama_pools.Corpus(
            short=["hi"], long=["a longer one"],
            code=[("python", "print(1)")], polls=[("q?", ["a", "b"])])
        restored = seed_ollama_pools.Corpus(**json.loads(json.dumps(original.as_dict())))
...
class FetchMessagesTest(unittest.TestCase):
    def test_parses_and_strips_the_message_list(self):
        with patch("urllib.request.urlopen",
            ...
            got = seed_ollama_pools._fetch_messages("http://x", "m", "prompt", 5)
    ...
    def test_a_malformed_response_raises_for_the_caller_to_handle(self):
        with patch("urllib.request.urlopen",
            ...
            with self.assertRaises(KeyError):
                seed_ollama_pools._fetch_messages("http://x", "m", "prompt", 5)
...
class FetchCodeTest(unittest.TestCase):
    def test_parses_lang_and_code_pairs(self):
        ...
        with patch("urllib.request.urlopen",
            ...
            got = seed_ollama_pools._fetch_code("http://x", "m", 5)
...
class FetchPollsTest(unittest.TestCase):
    def test_keeps_a_poll_with_two_to_four_options(self):
        ...
        with patch("urllib.request.urlopen",
            ...
            got = seed_ollama_pools._fetch_polls("http://x", "m", 5)
...
class FetchOrEmptyTest(unittest.TestCase):
    def test_returns_the_fetch_result_on_success(self):
        got = seed_ollama_pools._fetch_or_empty("thing", lambda: ["a"])
        self.assertEqual(got, ["a"])

    def test_any_exception_becomes_an_empty_list_not_a_raise(self):
        def boom():
            raise urllib.error.URLError("nope")
        got = seed_ollama_pools._fetch_or_empty("thing", boom)
        self.assertEqual(got, [])


class LoadOrGenerateTest(unittest.TestCase):
    def setUp(self):
        self.cache_dir = tempfile.mkdtemp(prefix="seed-ollama-pools-test-")
        self.addCleanup(shutil.rmtree, self.cache_dir, ignore_errors=True)

    def test_an_unreachable_server_never_calls_generate(self):
        with patch("seed_ollama._reachable", return_value=False), \
             patch("seed_ollama_pools.generate") as fake_generate:
            got = seed_ollama_pools.load_or_generate(
    ...
    def test_a_successful_generation_is_written_to_the_cache(self):
        corpus = seed_ollama_pools.Corpus(short=["hi"])
        with patch("seed_ollama._reachable", return_value=True), \
             patch("seed_ollama_pools.generate", return_value=corpus):
            seed_ollama_pools.load_or_generate("m", "seed-1", cache_dir=self.cache_dir)
        path = seed_ollama_pools.seed_ollama._cache_path("m", "seed-1", self.cache_dir)
    ...
    def test_a_second_call_with_the_same_model_and_seed_reads_the_cache(self):
        ...
        with patch("seed_ollama._reachable") as fake_reachable, \
             patch("seed_ollama_pools.generate") as fake_generate:
            got = seed_ollama_pools.load_or_generate("m", "seed-1", cache_dir=self.cache_dir)
    ...
    def test_a_different_seed_does_not_share_the_cache(self):
        ...
        with patch("seed_ollama._reachable", return_value=True), \
            ...
            seed_ollama_pools.load_or_generate("m", "seed-1", cache_dir=self.cache_dir)
        with patch("seed_ollama._reachable", return_value=False):
            got = seed_ollama_pools.load_or_generate("m", "seed-2", cache_dir=self.cache_dir)
        self.assertTrue(got.is_empty())

    def test_generation_returning_nothing_usable_is_not_cached(self):
        with patch("seed_ollama._reachable", return_value=True), \
             patch("seed_ollama_pools.generate", return_value=seed_ollama_pools.Corpus()):
            seed_ollama_pools.load_or_generate("m", "seed-1", cache_dir=self.cache_dir)
        path = seed_ollama_pools.seed_ollama._cache_path("m", "seed-1", self.cache_dir)
