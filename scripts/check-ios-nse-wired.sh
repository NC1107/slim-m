...
[ "$point" = "com.apple.usernotifications.service" ] || {
  echo "::error::$ext/Info.plist is not a notification service extension"
  fail=1
}
...
[ -n "$group" ] || {
  echo "::error::persistent_key_store.dart has no pushKeychainAccessGroup"
  fail=1
}
if [ -n "$group" ]; then
  ext_group=$(keychain_group_in "$ext/NotificationService.entitlements")
  [ "$ext_group" = "$group" ] || {
    echo "::error::the extension claims '$ext_group', not '$group'"
    fail=1
  }
  ...
  [ "$first" != "$group" ] || {
    echo "::error::Runner.entitlements lists '$group' first, which would"
    echo "::error::make it the default group every other secret is written to"
    fail=1
  }
  grep -q "<string>$group</string>" "$ios/Runner/Runner.entitlements" || {
    echo "::error::Runner.entitlements does not claim '$group'"
    fail=1
  }
  grep -q "\"$group\"" "$ext/PushKeychain.swift" || {
    echo "::error::PushKeychain.swift does not use '$group'"
    fail=1
  }
  ...
  grep -q 'groupId: pushKeychainAccessGroup' \
    "$dart/persistent_key_store.dart" || {
    echo "::error::the push key store does not write into \$group"
    fail=1
  }
fi
...
[ -n "$handle" ] || {
  echo "::error::device_push_keys.dart has no devicePushKeyHandle"
  fail=1
}
if [ -n "$handle" ]; then
  grep -q "\"$handle\"" "$ext/PushKeychain.swift" || {
    echo "::error::PushKeychain.swift does not read '$handle'"
    fail=1
  }
fi
...
grep -q '"flutter_secure_storage_service"' "$ext/PushKeychain.swift" || {
  echo "::error::PushKeychain.swift does not use the default kSecAttrService"
  fail=1
}
if grep -q 'accountName' "$dart/persistent_key_store.dart"; then
  echo "::error::persistent_key_store.dart sets accountName, which changes"
  echo "::error::kSecAttrService; PushKeychain.swift would then find nothing"
  fail=1
fi
...
grep -q 'first_unlock_this_device' "$dart/persistent_key_store.dart" || {
  echo "::error::the push key store is not first_unlock_this_device;"
  echo "::error::it would be unreadable exactly when a push arrives locked"
  fail=1
}
grep -q 'kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly' \
  "$ext/PushKeychain.swift" || {
  echo "::error::PushKeychain.swift queries a different accessibility"
  fail=1
}
...
if [ -n "$nse_id" ]; then
  ...
  [ "$count" -ge 3 ] || {
    echo "::error::$pbx does not build $nse_id in all three configs"
    fail=1
  }
  grep -q "<key>$nse_id</key>" "$ios/ExportOptions.plist" || {
    echo "::error::ExportOptions.plist has no profile for $nse_id"
    fail=1
  }
  ...
  echo "::error::$pbx builds no NotificationService bundle id"
  fail=1
fi
...
grep -q 'NotificationService.appex in Embed Foundation Extensions' \
  <<<"$copy_phases" || {
  echo "::error::$pbx does not embed NotificationService.appex"
  fail=1
}
...
if [ -z "$sources_id" ]; then
  echo "::error::$pbx has no Sources phase on the NotificationService target"
  fail=1
else
  ...
  for f in "$ext"/*.swift; do
    grep -qF "$(basename "$f") in Sources" <<<"$own" || {
      echo "::error::$(basename "$f") is not in NotificationService's own Sources"
      fail=1
    }
  done
fi
