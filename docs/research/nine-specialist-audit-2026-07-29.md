...
`SettingsScreenScaffold` closes the next one: the eight screens that carried the same `Scaffold`/`AppBar`/`AppContentColumn`/`SafeArea` frame verbatim - down to the same one-line comment about `top: false` - now name it once.
Worth recording how that went, because it is the argument for having the tests: converting the personal settings body from a `ListView` to a `Column` silently dropped the trailing `AppInfoSection`, and `settings_taxonomy_test` failed on exactly that.
A mechanical migration is the shape of change most likely to lose something quietly, which is why it wants a suite rather than a careful eye.

Recorded from the audit, not yet done: a `SettingsScreenScaffold` for eight verbatim-duplicated screen skeletons, `runGuarded` (built, with its first adopter): the twenty-six copies each interpolated the exception, so a transport failure put `POST /invites/x/revoke failed: SocketException...` in front of a user.
The helper maps by exception type instead - a `BadRequest` carries the server's own explanation because that is worth reading, a transport failure says the server could not be reached and that nothing was changed - and hands the sentence back for the caller to render where the action was, rather than showing a toast.
`run_guarded_test.dart` pins exactly that: no failure may contain the exception's own string.
