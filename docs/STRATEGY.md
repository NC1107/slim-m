...
Versioning: release-please in manifest mode with two independently versioned packages in the monorepo (server and client) plus a third standalone instance in the relay repo, with `bump-minor-pre-major` and `bump-breaking-pre-major` set true so self-hosters get a real pre-1.0 compatibility signal.
...
- Account deletion is a mandatory wire-protocol verb implemented in the reference server from v1, always visible client-side, with a non-hiding fallback for third-party servers; deletion purges the account's own personal data and tombstones the user, transfers group ownership, and revokes devices, satisfying 5.1.1(v)'s substance (meaningfully gone, not deactivated), not just the presence of a verb, while keeping authored messages as removed-author tombstones for other participants' history integrity.
