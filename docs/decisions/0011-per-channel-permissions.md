...
5. **`canvas_pane.dart:369-370`, canvas context menu and "Clear canvas."** Convert.
   `ref.watch(meProvider).valueOrNull?.permissions` becomes `ref.watch(channelPermissionsProvider(widget.channelId))`, read in the same `build()`, passed down as the same plain `bool canManage` constructor parameter, never touching the paint path.
