...
LiveKit's proto names the screen-share track sources `SCREEN_SHARE` and `SCREEN_SHARE_AUDIO`. Documentation for the exact JSON string LiveKit's Go server emits for these disagreed across sources consulted while building this (some rendered them without the `SHARE` segment), so the matching in `voice/webhook.rs` and `voice/roster.rs` checks for the substring `SCREEN` in the reported source rather than an exact literal, and the e2e run in this PR's description is what confirms the pinned server (`livekit/livekit-server:v1.10.1`) actually sends against this code path.
