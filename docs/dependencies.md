...
The Flatpak manifest (`packaging/flatpak/top.npcserver.slimm.yaml`) is a known gap this pass did not close: `org.freedesktop.Platform` carries no `libmpv` (and may or may not already carry `libepoxy` - a common enough GL dependency that it plausibly does), and nothing in this change adds a build module for either the way `libayatana-appindicator3` already gets one for the tray icon. Given the plugin now links both, this build may fail to launch at all rather than merely fail to play video, until a real `mpv` (or `libmpv`-only) build module is added; the rpm and the plain release tarball are unaffected, since both rely on a system package rather than the Flatpak sandbox's own runtime.
