...
- ~~The DM message-action menu can offer Pin and Delete to an administrator inside any DM even though the server's DM permission model grants neither to anyone, a systemic client/server permission-gating gap that also reaches threads and was the exact DM-authorization question this review set out to check.~~ Already closed on main before this pass (PR #522); see Cross-cutting below.
- ~~A thread panel gives no orientation once you're inside it: no channel name, no parent message, just the word "Thread" - the single biggest orientation gap found in the whole shell pass.~~ Fixed 2026-08-10 (title only, not a pinned parent message; see Thread below).
- ~~The icon that restores a collapsed channel rail is the sole way back and fails WCAG non-text contrast by roughly half the required ratio in both themes, measured directly from the rendered pixels.~~ Fixed 2026-08-10.
- ~~The composer's placeholder wraps to a second line at the exact width the two-pane rail-plus-content layout begins, found independently by the frontend and UX passes.~~ Fixed 2026-08-10.
- ~~A thread opened cold, by URL rather than by "Reply in thread," never gets its local read marker synced from the server, so the unread divider sits above the first message every session even though the server's own record is correct.~~ Fixed 2026-08-10.
- ~~Two smaller identity mix-ups: the DM header reuses the text-channel "#" glyph even though the code already branches on `isDm` two lines away, and the channel header truncates the channel's own name before its topic subtitle at the one width where both compete for space.~~ Both fixed 2026-08-10.
...
~~**Composer placeholder wraps to two lines at the exact width the two-pane rail-plus-content layout begins**, even though the identical text fits on one line at every other width tested.
  ...
  Fixed 2026-08-10: `composer_extras.dart`'s hint `Text.rich` now carries `maxLines: 1, overflow: TextOverflow.ellipsis`.
...
~~**The channel header truncates the channel's own name before its topic subtitle, the wrong way round for which one carries identity.**
  ...
  Fixed 2026-08-10: the name is now `Flexible(flex: 2, maxLines: 1, ...)` and the topic a plain (implicit flex 1) `Flexible`, so the name wins the race for space at every width.
...
~~**The thread panel's header says only "Thread," with no channel name and no parent message shown anywhere.**
  ...
  Fixed 2026-08-10: a new `GET /channels/{channelId}/thread-parent` route (masked to all-null unless the caller holds `VIEW_CHANNEL`, the same `mask_unless_viewable` rule decision 0011 established) answers the parent channel's id and name; `ThreadScreen`'s title is "Thread in #general" once it resolves, falling back to plain "Thread" only while in flight, on error, or for a parent this fetch genuinely cannot resolve. Pinning the parent message itself is not done here - a smaller, more targeted fix than the report's full suggestion.
...
~~**A thread opened cold, by URL rather than by "Reply in thread," never gets its local read marker hydrated from the server, so the "NEW" divider sits above the very first message every time the thread is reopened this session, regardless of what was actually read moments before.**
  ...
  Fixed 2026-08-10, by the same route above: `ThreadScreen._ensureThreadChannelRow` upserts a real local channel row, carrying the real `parentMessageId` the fetch resolved, the moment `GET .../thread-parent` answers - which both gives the read marker somewhere to write and keeps `replaceChannels`'s pruning rule from ever mistaking a cold-opened thread for one that dropped off the server's list.
...
~~A thread also inherits the message-action permission-gating gap named in Cross-cutting, since a thread's effective permissions resolve to its parent channel's overwrites.~~ See Cross-cutting: closed already.
...
~~- **The context menu can offer Pin and Delete-another-participant's-message inside a DM to any account holding `MANAGE_MESSAGES`/`ADMINISTRATOR` at the deployment level, even though the server's DM permission model grants neither to anyone, unconditionally.**
  ...
  Severity: high.~~
  Already closed on main, not part of this change: see Cross-cutting.
...
~~**The DM header uses the same "#" hash glyph as a text channel** ("# Ada Lovelace"), even though the rail row for the same conversation correctly shows the person's avatar instead.
  ...
  Fixed 2026-08-10: the icon is now `isVoice ? AppIcons.voice : (isDm ? AppIcons.account : AppIcons.hash)`, the branch the report pointed at.
...
~~**The empty content pane is a single small line of grey text, "Pick a channel to start reading," centred with no icon and no next step offered - noticeably less than `channel-genuinely-empty` gets for what is, functionally, the same "nothing to show yet" situation.**
  ...
  Fixed 2026-08-10: `_NothingSelected` now carries an icon, a heading and a subtitle matching `ChannelStartHeader`'s own weight, naming the actual next step ("Pick a channel from the list, or press Ctrl+K to jump to one" on a pointer layout).
...
~~**The restore-rail icon is a small, unlabelled outline sitting alone in a large empty field, and its contrast against the background fails WCAG non-text contrast by a wide margin.**
  ...
  Fixed 2026-08-10: the collapsed icon is now `AppTokens.textSecondary` rather than `borderSubtle`, which already clears the stricter 4.5:1 AA text floor in every theme and so clears the 3:1 UI-component floor with room, while staying the same muted, undecorated register. The hover/pill suggestion is not built - the icon still has no hit-target boundary beyond its own glyph at rest.
...
~~**The client has no mechanism anywhere to read a per-channel effective permission, and this is the systemic cause behind the DM finding above.**
  ...
  Already closed on main, not part of this change: `channel_screen.dart`'s `myPermissions` now reads `myChannelPermissionsProvider(widget.channelId)` ("site 1" per its own inline comment), part of the decision-0011 per-channel-permissions sweep (PR #522, see [docs/decisions/0011-per-channel-permissions.md](../../decisions/0011-per-channel-permissions.md) and CLAUDE.md's "The client asked one permission question and the server answered a different one, in eight places," 2026-08-10) that landed before this pass started. Checked directly against current source, not assumed stale from the report text alone.
