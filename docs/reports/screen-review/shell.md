...
~~**The client has no mechanism anywhere to read a per-channel effective permission, and this is the systemic cause behind the DM finding above.**
  ...
  Already closed on main, not part of this change: `channel_screen.dart`'s `myPermissions` now reads `myChannelPermissionsProvider(widget.channelId)` ("site 1" per its own inline comment), part of the decision-0011 per-channel-permissions sweep (PR #523, see [docs/decisions/0011-per-channel-permissions.md](../../decisions/0011-per-channel-permissions.md) and CLAUDE.md's "The client asked one permission question and the server answered a different one, in eight places," 2026-08-10) that landed before this pass started. Checked directly against current source, not assumed stale from the report text alone.
