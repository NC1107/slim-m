...
Fixed in the same PR that introduced this workflow, so as of the PR being open, the corrected manifest has not yet completed a green run; that is what the run linked from the PR confirms or does not.
Once it is green, the build itself (the harder half of this check, and the half that just proved it can catch a real defect) is confirmed for this exact `flatpak-builder`/CMake/sandbox combination.
