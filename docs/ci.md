...
Locally verified: the YAML parses, `actionlint` reports no findings, every command mirrors a step `release.yml`'s own `linux-client` job already runs successfully in CI (the `elfutils` fix from #967 included), and the `dbus-daemon`/`dbus-run-session` package claim above is checked against Ubuntu's own package-contents search, not assumed.
Not verified: `flatpak-builder` was not available to actually build and launch this exact bundle while writing this workflow, so the `124`-vs-any-other-nonzero-exit split is reasoned from documented `timeout` and `flatpak run` behavior, not confirmed against a live run of this bundle.
That means it is not yet confirmed that `flatpak run` actually reaches the plugin-loading stage within the 20-second timeout on a GitHub-hosted runner with no display attached at all (unlike the author's own machine, which has a real session), that unprivileged flatpak sandboxing itself works unmodified on `ubuntu-latest`'s current image, or that some other headless-environment quirk unrelated to a missing shared library (a portal or D-Bus service genuinely absent in that runner) does not also exit nonzero and trip the exit-code assertion.
If the first real run fails on exactly that shape, the fix is to loosen the exit-code assertion, not the two `grep` patterns, which are the actual defect this workflow exists to catch.
