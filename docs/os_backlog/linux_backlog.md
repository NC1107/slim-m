...
*Not confirmed*: that the resulting rpm and flatpak, which repackage `release.yml`'s already-built bundle rather than rebuilding, correctly declare or already satisfy the new `libpulse-simple.so.0` runtime dependency this pulls in - the rpm's existing `Requires: libpulse.so.0()(64bit)` is for a different soname (libwebrtc's own audio device module), and this was not independently re-verified with `elfdeps`/`rpm -qpR` the way the tray-icon dependency work above was. Worth checking before relying on the packaged (as opposed to the portable tarball) Linux build having this.
